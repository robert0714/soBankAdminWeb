...
@ActiveProfiles("test")
public class RoleServiceForJPAImplTest   extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void testFindAll() throws Exception {
		List<RolePo> result = service.findAll();
	...
	public void testFindById() throws Exception {
		...
		RolePo po = service.findById(result.get(0).getId());
		assertNotNull(po);
	}
	@DataSets(setUpDataSet= "/com/iisigroup/product/secure/service/secureTest.xls")
	@Test
	public void testSave() throws Exception {
		...
		po.setRoleCode("user01");
		po.setRoleDesc("Test User Name");

        service.save(po);
        em.flush();

        List<RolePo> pos = service.findAll();
	...
	public void testFindAllByPage() throws Exception {
		...
		Page<RolePo> page = service.findAllByPage(pageable);
		final int size = page.getNumberOfElements();
		ArrayList<RolePo> list = Lists.newArrayList(page.iterator());
		assertNotNull(list);
		assertEquals(1, size);
	}
}
